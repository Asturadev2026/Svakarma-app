...
export class CibilService {
  async getCibilData(userId: string, generate = false) {
    let scores = await prisma.cibilScore.findMany({
      where: { userId },
    ...
    if (scores.length === 0) {
      ...
      const newScore = await prisma.cibilScore.create({
        data: {
          userId,
          score: initialScore,
    ...
    let status = 'Fair';
    if (latestScore >= 750) status = 'Excellent';
    else if (latestScore >= 700) status = 'Good';
    else if (latestScore < 600) status = 'Poor';
